...
export function parsePhotoManifest(value: unknown): PhotoManifest {
  if (!isRecord(value) || Object.keys(value).some(key => !['type', 'size', 'revision', 'profile', 'crop'].includes(key)) ||
    ...
    (value.crop !== undefined && !isPhotoCrop(value.crop))) throw new Error('invalid_photo');
