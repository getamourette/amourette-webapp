...
export function VenueNights({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <section className="night-panel order-2 rounded-3xl p-5">
        <p className="night-kicker mb-2">Open · launch · close</p>
        <h2 className="mb-1 text-xl font-bold">{editing ? "Edit opening times" : "Schedule opening times"}</h2>
        {embedded && <p className="night-muted mb-4 text-sm">Choose a venue once, then set when guests can enter, when the room launches, and when it closes.</p>}
      ...
      const consequence = state === "Scheduled"
        ...
        : state === "Waiting"
          ...
          : state === "Paused"
            ...
            : "";
      return <li key={night.id} className="night-card rounded-2xl p-4"><div className="flex flex-wrap justify-between gap-4"><div><div className="flex items-center gap-2"><strong>{itemVenue?.name}</strong><span className="night-pill rounded-full px-3 py-1 text-xs">{state}</span></div><div className="mt-3 grid gap-2 text-sm sm:grid-cols-3"><div><p className="night-kicker mb-1">Entry opens</p><p>{formatVenueInstant(night.waiting_opens_at, itemVenue.timezone)}</p></div><div><p className="night-kicker mb-1">Guaranteed launch</p><p>{formatVenueInstant(night.guaranteed_launch_at, itemVenue.timezone)}</p></div><div><p className="night-kicker mb-1">Closes</p><p>{formatVenueInstant(night.closes_at, itemVenue.timezone)}</p></div></div>{state === "Waiting" && <p className="mt-3 text-sm">{counts[night.id] ?? 0} / {night.launch_threshold} participants · launches earlier if the threshold is reached</p>}{consequence && !isHistory && <p className="night-muted mt-2 max-w-xl text-xs">{consequence}</p>}</div>{!isHistory && <div className="flex flex-wrap gap-2">{!isQa && state === "Scheduled" && <><button disabled={isBusy} onClick={() => editNight(night)} className="night-button night-button-secondary px-3 py-2 text-xs">Edit times</button><button disabled={isBusy} onClick={() => void act(night, "open")} className="night-button night-button-primary px-3 py-2 text-xs">Open waiting now</button></>}{!isQa && state === "Waiting" && <button disabled={isBusy} onClick={() => void act(night, "launch")} className="night-button night-button-primary px-3 py-2 text-xs">Launch now</button>}{["Waiting", "Live"].includes(state) && <button disabled={isBusy} onClick={() => setConfirming({ night, action: "close" })} className="night-button night-button-secondary px-3 py-2 text-xs">Close temporarily</button>}{state === "Paused" && Date.parse(night.closes_at) > Date.now() && <button disabled={isBusy} onClick={() => void act(night, "reopen")} className="night-button night-button-primary px-3 py-2 text-xs">Reopen</button>}{!isQa && ["Scheduled", "Waiting", "Live", "Paused"].includes(state) && <button disabled={isBusy} onClick={() => setConfirming({ night, action: "cancel" })} className="night-button night-button-danger px-3 py-2 text-xs">Cancel</button>}</div>}</div><details className="mt-4 border-t border-champagne/10 pt-3"><summary className="cursor-pointer text-xs font-semibold">Audit timeline ({timeline.length})</summary><ol className="night-muted mt-2 space-y-2 text-xs">{timeline.map((event, index) => <li key={`${event.at}-${index}`}><span className="text-cream">{event.label}</span> · {event.actor} · {new Date(event.at).toLocaleString()}<br />{event.detail}</li>)}</ol></details></li>;
    })}</ul>}</div>;
  }
}
