...
export function PhotoQueue({ reportProfileId, onCloseReport }: { reportProfileId?: string | null; onCloseReport?: () => void }) {
  ...
  async function decide(version: string, action: 'approved' | 'rejected') {
    ...
    if (result.error) {
      setMessage(result.error.code === '40001' ? 'This review changed while you were looking. The latest photos are loaded below; review them again.' : 'Could not save this decision. Please try again.');
