...
import { isBioLengthError, isRecord } from '@/lib/input-validation';
import { PHOTO_STAGING_BUCKET, type PhotoCrop } from '@/lib/photo-upload';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (request.headers.get('content-type')?.split(';')[0] === 'application/json') {
      ...
      if (!isRecord(body) || Object.keys(body).length !== 1) return Response.json({}, { status: 400 });
      ...
      revision = ticket.revision; profileData = ticket.profile; crop = ticket.crop;
    } else {
