...
- **Bloc 1, QR check-in and live presence**: `/v/[venueSlug]` route, dashboard scoped to who is checked in here now, presence that fades on leave/timeout. Also (small, do first): centralize the hardcoded UI strings into a FR/EN dictionary (`lib/strings.ts`), locale derived from `venues.city` (Paris → fr, NYC → en) with browser-language fallback on pre-venue pages — no i18n framework yet (see `docs/decisions.md`, 2026-06-19).
