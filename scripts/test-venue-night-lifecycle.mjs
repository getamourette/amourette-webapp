...
try {
  ...
  equal((await select(service.from("messages").select("id").eq("match_id", matches[0].id))).length, 1, "departure preserves messages");
  await rpc(clients[1], "check_in", { p_venue_id: venue.id });
  ...
  equal((await select(clients[1].from("messages").select("id").eq("match_id", matches[0].id))).length, 1, "re-entry restores preserved messages");
