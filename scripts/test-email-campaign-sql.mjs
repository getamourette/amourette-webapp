...
await db.exec(read('20260728000001_canonical_email_subscriptions').split('-- Legacy room consents')[0]);
await db.exec(read('20260802000001_resend_email_delivery_foundation').split('-- The application worker is called')[0]);
await db.exec(read('20260909000003_input_validation_contract').split('-- Runs before existing workflow triggers:')[0]);
await db.exec(read('20260921000002_admin_email_campaigns'));
