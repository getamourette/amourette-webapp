...
export function ProfilePhoto({ src, profileId, alt = '', ...props }: Omit<ImgHTMLAttributes<HTMLImageElement>, 'src'> & { src?: string | null; profileId?: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (active) setLoaded({ source: src, url, epoch, profileId });
    })();
    return () => { active = false; if (blobUrl) URL.revokeObjectURL(blobUrl); };
  }, [src, profileId, epoch, inView]);
  const url = loaded?.source === src && loaded?.epoch === epoch && loaded.profileId === profileId && loaded.url !== failedUrl ? loaded.url : null;
